...
`package.json` on 15 August 2026. This table is the source of truth for the
policy pages — if an app changes, update this first, then the page.

| | BDT Rate Today | GPA Calculator BD | শব্দ খেলা |
|---|---|---|---|
| Version | 1.0.0 | 1.0.1 | 1.0.0 |
| Release permissions | `INTERNET` | `VIBRATE` | `VIBRATE` |
| Network | 4 public rate APIs | none | none |
| On-device storage | none | MMKV `files/mmkv` | MMKV `files/mmkv` |
| `android:allowBackup` | `false` | `true` | `true` |
| Export / import | — | JSON via share sheet + document picker | JSON via share sheet + document picker |
| Share as image | — | result card, scale JSON | result card |

Every release build also carries
`<package>.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION`, generated by AndroidX. It
is signature-level, grants nothing, and is disclosed on the two pages where a
...
- **Permissions** — read the *merged release* manifest, not just
  ...
  own manifest. `VIBRATE` reaches both offline apps this way, from
...
- **`android:allowBackup`** — if it is `true` (the Android default), the OS may
  ...
  other two leave it on and ship `backup_rules.xml` /
